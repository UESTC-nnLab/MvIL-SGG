...
def self_supervise_aug1(scene_points, instances):
    ...
    elastic_config = {
        'granularity': (4, 4, 4),  # 在x,y,z轴上都分成4段
        'magnitude': (0.12, 0.12, 0.12)  # 各轴位移幅度
    }
    ...
    for in_id in instance_ids:
        ...
        dropout_ratio = random.uniform(0.4, 0.75)
...
class HonsenDatasetGraph(data.Dataset):
    def __init__(self,
        ...
        self.sample_list = constructSamplelist1(self.graph_labels)
    ...
    def __getitem__(self, index):
        ...
        selected_rels = self.graph_labels[scene_group_id]

        aug_points1, aug_instances1 = self_supervise_aug1(points.copy(), instances.copy())

        obj_points, rel_points, edge_indices, descriptor = \
            self.data_preparation(aug_points1, aug_instances1, self.num_points, self.num_points_union, selected_rels,
                 padding=0.2)

        aug_points2, aug_instances2 = self_supervise_aug1(points.copy(), instances.copy())

        rotated_obj_points, rotated_rel_points, _, rotated_descriptor = \
            self.data_preparation(aug_points2, aug_instances2, self.num_points, self.num_points_union, selected_rels,
                                  padding=0.2)

        while (len(rel_points) == 0) and self.for_train:
            index = np.random.randint(self.__len__())
            if self.use_descriptor:
                (obj_points, rel_points, descriptor), (rotated_obj_points, rotated_rel_points, rotated_descriptor), edge_indices, scene_group_id = self.__getitem__(index)
            else:
                obj_points, rel_points, gt_rels, edge_indices = self.__getitem__(index)

        if self.use_descriptor:
             return (obj_points, rel_points, descriptor), (rotated_obj_points, rotated_rel_points, rotated_descriptor), edge_indices, scene_group_id

        return obj_points, rel_points, edge_indices, scene_group_id
