...
def collate_fn_mmg(batch):
    # batch
    obj_point_list, rot_obj_point_list = [], []
    edge_indices, descriptor, rot_descriptor = [], [], []
    batch_ids = []
    clip_feature_labels = []
    scan_ids = []
    count = 0
    #  return obj_points, rel_points, edge_indices, descriptor
    # b0:ori b1:rot b2:edge_idx b3:clip_feat b4:scene_id
    for i, b in enumerate(batch):

        obj_point_list.append(b[0][0])
        rot_obj_point_list.append(b[1][0])

        descriptor.append(b[0][2])
        rot_descriptor.append(b[1][2])

        edge_indices.append(b[2] + count)
        clip_feature_labels.append(b[3])

        # accumulate batch number to make edge_indices match correct object index
        count += b[0][0].shape[0]

        # get batchs location
        batch_ids.append(torch.full((b[0][0].shape[0], 1), i))
        scan_ids.append(b[4])

    return torch.cat(obj_point_list, dim=0), torch.cat(descriptor, dim=0), torch.cat(rot_obj_point_list, dim=0), torch.cat(rot_descriptor, dim=0),\
           torch.cat(edge_indices, dim=0), clip_feature_labels, torch.cat(batch_ids, dim=0)

def collate_fn_mmg_single(batch):
    # batch
    obj_point_list, rot_obj_point_list = [], []
    edge_indices, descriptor, rot_descriptor = [], [], []
    batch_ids = []
    ...
    return torch.cat(obj_point_list, dim=0), torch.cat(descriptor, dim=0), torch.cat(rot_obj_point_list, dim=0), torch.cat(rot_descriptor, dim=0),\
           torch.cat(edge_indices, dim=0),  torch.cat(batch_ids, dim=0)

#obj_points, rot_obj_points, rel_points, gt_class, gt_rels, edge_indices, descriptor, rot_descriptor, scan_id
def collate_fn_mmg1(batch):
    # batch
    obj_point_list, rot_obj_point_list = [], []
    edge_indices, descriptor, rot_descriptor = [], [], []
    batch_ids = []
    ...
    return torch.cat(obj_point_list, dim=0), torch.cat(descriptor, dim=0), torch.cat(rot_obj_point_list, dim=0), torch.cat(rot_descriptor, dim=0),\
           torch.cat(edge_indices, dim=0), torch.cat(batch_ids, dim=0), torch.cat(obj_label_list, dim=0), torch.cat(rel_label_list, dim=0), torch.cat(rel_ids, dim=0)