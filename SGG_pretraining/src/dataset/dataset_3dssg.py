...
class SSGDatasetGraph(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        points = data['points']
        # visualPoints_plt(points)

        verticalT = [[8.87756050e-01, 1.2, 0.0, 0.00000000e+00],
                     [-1.2, 8.87758732e-01, 0.0, 0.00000000e+00],
                     [0.0, 0.0, 9.99995649e-01, 0.00000000e+00],
                     [0.0, 0.0, 0.0, 1.00000000e+00]]
        verticalT = np.array(verticalT)

        instances = data['instances']
        points_hom = np.hstack([points, np.ones((points.shape[0], 1))])

        w_2_c = (verticalT @ points_hom.T)  # (N, 4)

        points = w_2_c.transpose(1, 0)[:, 0:3]  # n_frames x n_points x 3
        # visualPoints_plt(points)
        ...
        if self.use_descriptor:
            if self.use_2d_feats:
                return obj_points, obj_2d_feats, rel_points, gt_class, gt_rels, edge_indices, descriptor, scan_id  #/////////////
