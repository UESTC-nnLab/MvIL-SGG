...
class SSGDatasetGraph1(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        aug_points, aug_instances = self_supervise_aug(points.copy(), instances.copy())
        ...
        obj_points, rel_points, gt_rels, gt_class, edge_indices, descriptor,rel_id = \
            self.data_preparation(points, instances, 512, self.mconfig.num_points_union,
        ...
        rot_obj_points, _, _, _, _, rot_descriptor,rel_id = \
            self.data_preparation(aug_points, aug_instances, 512, self.mconfig.num_points_union,
                                  for_train=self.for_train, instance2labelName=map_instance2labelName,
                                  classNames=self.classNames,  # all class name in the dataset
                                  rel_json=self.relationship_json[scan_id],
                                  relationships=self.relationNames,
        ...
        while (len(rel_points) == 0 or gt_rels.sum() == 0) and self.for_train:
            ...
            if self.use_descriptor:
                if self.use_2d_feats:
                    ...
                    obj_points, rot_obj_points, rel_points, gt_class, gt_rels, edge_indices, descriptor, rot_descriptor, rel_id = self.__getitem__(
                        index)
            else:
        ...
        if self.use_descriptor:
            ...
            # print(gt_class)
            return obj_points, rot_obj_points, rel_points, gt_class, gt_rels, edge_indices, descriptor, rot_descriptor, rel_id
