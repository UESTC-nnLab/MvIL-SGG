...
from input_Trans import convert_bcn_to_pt_input, convert_pt_input_to_bcn_vectorized
...
class PointTransformerSeg(nn.Module):
    ...
    def forward(self, points):
        pxo = convert_bcn_to_pt_input(points)
        p0, x0, o0 = pxo['coord'],pxo['feat'],pxo['offset'] # (n, 3), (n, c), (b)
        ...
        pxo['coord'], pxo['feat'], pxo['offset'] = p5, x5, o5
        encoder_points = convert_pt_input_to_bcn_vectorized(pxo)
...
if __name__ =="__main__":
    model = pointtransformer_seg_repro().cuda()
    # feats = torch.rand((2*1024, 3), dtype=torch.float)
    points = torch.rand((3, 3, 1024), dtype=torch.float).cuda()
    # b = torch.tensor([0,512,1024], dtype=torch.int).cuda()
    output = model(points)
    print()