...
class GraphEdgeAttenNetwork(torch.nn.Module):
    def __init__(self, num_heads, dim_node, dim_edge, dim_atten, aggr= 'max', use_bn=False,
                 flow='target_to_source',attention = 'fat',use_edge:bool=True, adapter = False, **kwargs):
        ...
        self.edgeatten = MultiHeadedEdgeAttention(
            ...
            num_heads=num_heads,use_bn=use_bn,attention=attention,use_edge=use_edge, adapter=adapter, **kwargs)
...
class MultiHeadedEdgeAttention(torch.nn.Module):
    def __init__(self, num_heads: int, dim_node: int, dim_edge: int, dim_atten: int, use_bn=False,
                 attention = 'fat', use_edge:bool = True, adapter=False, **kwargs):
        super().__init__()
        ...
        if adapter:
            self.adapter = True
            self.adapter_net = build_mlp([dim_node*2+dim_edge,(dim_node+dim_edge),dim_edge],
                          do_bn= use_bn, on_last=False)
            self.knowledge_filter = nn.Sequential(nn.Linear(512 * 2, 1), nn.Sigmoid())
        self.mask_obj = 0.5
    ...
    def forward(self, query, edge, value, weight=None, istrain=False):
        ...
        if self.adapter:
            f_Gate = self.knowledge_filter(torch.cat([self.nn_edge(edge_feature),self.adapter_net(edge_feature)],dim=1))
            # edge_feature = self.nn_edge(edge_feature)+self.adapter_net(edge_feature)
            edge_feature = f_Gate*self.nn_edge(edge_feature)+(1-f_Gate)*self.adapter_net(edge_feature)#.view(b, -1, 1)
        else:
...
class MMG(torch.nn.Module):
    ...
    def __init__(self, dim_node, dim_edge, dim_atten, num_heads=1, aggr= 'max', 
                 ...
                 hidden_size=512, depth=1, use_edge:bool=True, adapter = False, **kwargs,
                 ):
