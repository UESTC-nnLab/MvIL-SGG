...
class PointNetfeat(BaseNetwork):
    def __init__(self, global_feat = True, input_transform = True, feature_transform = False, 
                 ...
                 init_weights=True, pointnet_str:str=None, adapter = False):
        ...
        self.conv1 = torch.nn.Conv1d(point_size, 64, 1)
        self.conv2 = torch.nn.Conv1d(64, 128, 1)
        self.adapter3 = nn.Sequential(nn.Conv1d(128, out_size,1), nn.BatchNorm1d(out_size), nn.ReLU())

        self.adapter = adapter
        if adapter:
            self.adapter1 = nn.Sequential(nn.Conv1d(point_size, 64, 1), nn.BatchNorm1d(64), nn.ReLU())
            self.adapter2 = nn.Sequential(nn.Conv1d(64, 128,1), nn.BatchNorm1d(128), nn.ReLU())
            

            self.knowledge_filter1 = nn.Sequential(nn.Conv1d(64*2,1,1),nn.Sigmoid())
            self.knowledge_filter2 = nn.Sequential(nn.Conv1d(128 * 2, 1, 1), nn.Sigmoid())
            # self.knowledge_filter3 = nn.Sequential(nn.Conv1d(out_size * 2, 1, 1), nn.Sigmoid())
    ...
    def forward(self, x, return_meta=False):
        ...
        if self.adapter:
            gate1 = self.knowledge_filter1(torch.concat([self.adapter1(x),x1],dim=1))
            self.gate1list.append(gate1)
            x1 = (1-gate1)*self.adapter1(x)+gate1*x1

        if self.feature_transform:
            trans_feat = self.fstn(x1)
            x1 = x1.transpose(2,1)
            x1 = torch.bmm(x1, trans_feat)
            x1 = x1.transpose(2,1)
        else:
        ...
        if self.adapter:
            gate2 = self.knowledge_filter2(torch.concat([self.adapter2(x1), x2], dim=1))
            self.gate2list.append(gate2)
            x2 = (1 - gate2) * self.adapter2(x1) + gate2 * x2
        ...
        x= self.adapter3(x2)
        # if self.adapter:
        #     gate3 = self.knowledge_filter3(torch.concat([self.adapter3(x2), x3], dim=1))
        #     self.gate3list.append(gate3)
        #     x = (1 - gate3) * self.adapter3(x2) + gate3 * x3
        # else:
        #     x = x3
