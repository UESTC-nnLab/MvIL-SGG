...
from src.model.DGCNN import DGCNN
from src.model.model_utils.network_PointNet import (PointNetfeat,PointNetCls,
                                                    PointNetRelCls,
                                                    PointNetRelClsMulti)
...
class Mmgnet(BaseModel):
    def __init__(self, config, num_obj_class, num_rel_class, dim_descriptor=11, adapter = True):
        '''
        3d cat location, 2d
        '''
        ...
        # self.obj_encoder = DGCNN(k=20, emb_dims=512, output_channels=dim_point_feature)
        ...
        self.obj_encoder.load_state_dict(stat_dict1,strict=False)
        ...
        self.rel_encoder_3d = PointNetfeat(
            ...
            input_transform=False,
            feature_transform=mconfig.feature_transform,
            out_size=512, adapter=adapter)

        # self.rel_encoder_3d = DGCNN(k=20, emb_dims=512, inp_channels=8,output_channels=512)
        ...
        self.mmg = MMG(
            ...
            DROP_OUT_ATTEN=self.mconfig.DROP_OUT_ATTEN,adapter=adapter)
        ...
        self.obj_predictor_3d = PointNetCls(num_obj_class, in_size=512,
        ...
        mmg_obj, mmg_rel, mmg_adapter = [], [], []
        for name, para in self.mmg.named_parameters():
            if 'nn_edge' in name:
                mmg_rel.append(para)
            elif 'adapter' in name:
                mmg_adapter.append(para)
            else:
                mmg_obj.append(para)

        rel_encoder_noAda, rel_encoder_ada = [],[]
        for name, para in self.rel_encoder_3d.named_parameters():
            if 'adapter' in name:
                rel_encoder_ada.append(para)
            else:
                rel_encoder_noAda.append(para)

        # self.config.W_DECAY = 1e-5

        self.optimizer = optim.AdamW([
            {'params': self.obj_encoder.parameters(), 'lr': float(config.LR), 'weight_decay': self.config.W_DECAY,
             'amsgrad': self.config.AMSGRAD},
            {'params': self.rel_encoder_3d.parameters(), 'lr': float(config.LR), 'weight_decay': self.config.W_DECAY,
             'amsgrad': self.config.AMSGRAD},
            {'params': mmg_obj, 'lr': float(config.LR) / 4, 'weight_decay': self.config.W_DECAY,
             'amsgrad': self.config.AMSGRAD},
            {'params': mmg_rel, 'lr': float(config.LR) / 2, 'weight_decay': self.config.W_DECAY,
             'amsgrad': self.config.AMSGRAD},
            {'params': self.obj_predictor_3d.parameters(), 'lr': float(config.LR),
            ...
            {'params': self.rel_predictor_3d.parameters(), 'lr': float(config.LR), 'weight_decay': self.config.W_DECAY,
             'amsgrad': self.config.AMSGRAD},
            {'params': self.mlp_3d.parameters(), 'lr': float(config.LR), 'weight_decay': self.config.W_DECAY,
             'amsgrad': self.config.AMSGRAD},
            {'params': self.obj_logit_scale, 'lr': float(config.LR), 'weight_decay': self.config.W_DECAY,
             'amsgrad': self.config.AMSGRAD},
            # {'params': self.mlp_edge_forBT.parameters(), 'lr': float(config.LR),
            #  'weight_decay': self.config.W_DECAY,
            #  'amsgrad': self.config.AMSGRAD},
            # {'params': self.mlp_obj_forBT.parameters(), 'lr': float(config.LR) / 1.2, 'weight_decay': self.config.W_DECAY,
            #  'amsgrad': self.config.AMSGRAD},
        ])
        ...
        #     {'params': self.obj_encoder.parameters(), 'lr': float(config.LR), 'weight_decay': self.config.W_DECAY,
        #      'amsgrad': self.config.AMSGRAD},
        #     {'params': rel_encoder_noAda, 'lr': float(config.LR)*0.8, 'weight_decay': self.config.W_DECAY,
        #      'amsgrad': self.config.AMSGRAD},
        #     {'params': rel_encoder_ada, 'lr': float(config.LR), 'weight_decay': self.config.W_DECAY,
        #      'amsgrad': self.config.AMSGRAD},
        #     {'params': mmg_obj, 'lr': float(config.LR) / 4, 'weight_decay': self.config.W_DECAY,
        #      'amsgrad': self.config.AMSGRAD},
        #     {'params': mmg_rel, 'lr': float(config.LR) / 2, 'weight_decay': self.config.W_DECAY,
        #      'amsgrad': self.config.AMSGRAD},
        #     {'params': mmg_adapter, 'lr': float(config.LR), 'weight_decay': self.config.W_DECAY,
        #      'amsgrad': self.config.AMSGRAD},
        #     {'params': self.obj_predictor_3d.parameters(), 'lr': float(config.LR),
        #      'weight_decay': self.config.W_DECAY, 'amsgrad': self.config.AMSGRAD},
        #     {'params': self.rel_predictor_3d.parameters(), 'lr': float(config.LR), 'weight_decay': self.config.W_DECAY,
        ...
        #     {'params': self.obj_logit_scale, 'lr': float(config.LR), 'weight_decay': self.config.W_DECAY,
        #      'amsgrad': self.config.AMSGRAD},
        #     {'params': self.mlp_edge_forBT.parameters(), 'lr': float(config.LR), 'weight_decay': self.config.W_DECAY,
        #      'amsgrad': self.config.AMSGRAD},
        #     {'params': self.mlp_obj_forBT.parameters(), 'lr': float(config.LR), 'weight_decay': self.config.W_DECAY,
        #      'amsgrad': self.config.AMSGRAD},
        # ])
    ...
    def get_rel_emb(self, objs_target, rels_target, edges):
        ...
        for edge_index in range(len(edges)):
            ...
            if rels_target[edge_index].sum() == 0:
                target_rel_tokens.append(
                    clip.tokenize(f"the {target_eo} and the {target_os} has no relation in the point cloud"))
                ...
                for i in range(rels_target.shape[-1]):
                    if rels_target[edge_index][i] == 1:
                        ...
                        target_rel_tokens.append(
                            clip.tokenize(f"a point cloud of a {target_eo} {target_rel} a {target_os}"))
                        rel_index.append(edge_index)
    ...
    def forward(self, obj_points, edge_indices, descriptor=None, batch_ids=None, istrain=False,rel_points=None):

        obj_feature = self.obj_encoder(obj_points)

        obj_feature = self.mlp_3d(obj_feature[0])
        ...
        if rel_points is not None:
            rel_feature_3d = self.rel_encoder_3d(rel_points)
        else:
            ''' Create edge feature '''
            with torch.no_grad():
        ...
        gcn_obj_feature_3d, gcn_edge_feature_3d = self.mmg(obj_feature, rel_feature_3d, edge_indices, batch_ids, obj_center,
                       descriptor.clone(), istrain=istrain)

        # gcn_obj_feature_3d = self.mlp_obj_forBT(gcn_obj_feature_3d)
        # gcn_edge_feature_3d = self.mlp_edge_forBT(gcn_edge_feature_3d)

        rel_cls_3d = self.rel_predictor_3d(gcn_edge_feature_3d)

        logit_scale = self.obj_logit_scale.exp()

        obj_logits_3d = logit_scale * self.obj_predictor_3d(
            gcn_obj_feature_3d / gcn_obj_feature_3d.norm(dim=-1, keepdim=True))

        if istrain:
            return obj_logits_3d, rel_cls_3d, logit_scale
        else:
            return obj_logits_3d, rel_cls_3d

    def forward1(self, obj_points, edge_indices, descriptor=None, batch_ids=None, istrain=False):
        ...
        triplet_feats = self.generate_object_pair_features(gcn_obj_feature_3d,gcn_edge_feature_3d,edge_indices)
        return gcn_obj_feature_3d, gcn_edge_feature_3d, triplet_feats

    def process_train(self, obj_points, gt_cls, descriptor, gt_rel_cls, edge_indices, batch_ids=None,
                      with_log=False, ignore_none_rel=False, weights_obj=None, weights_rel=None,rel_points=None):
        self.iteration += 1

        obj_logits_3d, rel_cls_3d, obj_logit_scale = self(
            obj_points, edge_indices.t().contiguous(), descriptor, batch_ids, istrain=True, rel_points=rel_points)
        ...
        if self.mconfig.multi_rel_outputs:
            if self.mconfig.WEIGHT_EDGE == 'BG':
                ...
                weight = torch.abs(
                    1.0 / (torch.log(batch_mean + 1) + 1))  # +1 to prevent 1 /log(1) = inf
                if ignore_none_rel:
                    weight[0] = 0
        ...
        loss = lambda_o * (loss_obj_3d) + 3 * lambda_r * (loss_rel_3d)
        self.backward(loss)
        ...
        top_k_rel,_ = evaluate_topk_predicate(rel_cls_3d.detach(), gt_edges, self.mconfig.multi_rel_outputs, topk=6)
    ...
    def process_val(self, obj_points, gt_cls, descriptor, gt_rel_cls, edge_indices, batch_ids=None,
                    with_log=False, use_triplet=False):

        obj_logits_3d, rel_cls_3d = self(obj_points, edge_indices.t().contiguous(), descriptor,
                                                                    batch_ids, istrain=False)

        # compute metric
        top_k_obj = evaluate_topk_object(obj_logits_3d.detach().cpu(), gt_cls, topk=11)
        gt_edges = get_gt(gt_cls, gt_rel_cls, edge_indices, self.mconfig.multi_rel_outputs)

        top_k_rel, scene_accuracy = evaluate_topk_predicate(rel_cls_3d.detach().cpu(), gt_edges,
                                                            self.mconfig.multi_rel_outputs, topk=6)

        if use_triplet:
            top_k_triplet, cls_matrix, sub_scores, obj_scores, rel_scores = evaluate_triplet_topk(
                obj_logits_3d.detach().cpu(), rel_cls_3d.detach().cpu(), gt_edges, edge_indices,
                self.mconfig.multi_rel_outputs, topk=101, use_clip=True, obj_topk=top_k_obj)
        else:
            top_k_triplet = [101]
            cls_matrix = None
            sub_scores = None
        ...
        return top_k_obj, top_k_rel, top_k_triplet, cls_matrix, sub_scores, obj_scores, rel_scores, scene_accuracy
