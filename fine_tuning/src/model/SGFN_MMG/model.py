...
# from src.model.pointMAE.models.Point_MAE import PointTransformer as PointTransMAE_Encoder
from src.model.depthContrast.models.trunks.pointnet import PointNet
class Mmgnet(BaseModel):
    def __init__(self, config, num_obj_class, num_rel_class, dim_descriptor=11, adapter = True):
        ...
        self.mmg = MMG(
            ...
            DROP_OUT_ATTEN=self.mconfig.DROP_OUT_ATTEN,adapter=False)
        ...
        for name, para in self.mmg.named_parameters():
            if 'nn_edge' in name:
                mmg_rel.append(para)
            elif 'adapter_net' in name or 'knowledge_filter' in name:
                mmg_adapter.append(para)
            else:
                mmg_obj.append(para)

        rel_encoder_noAda, rel_encoder_ada = [],[]
        rel_name_noada, rel_name_ada = [],[]
        for name, para in self.rel_encoder_3d.named_parameters():
            if 'adapter' in name  or 'knowledge_filter' in name:
                rel_encoder_ada.append(para)
                rel_name_ada.append(name)
            else:
                rel_encoder_noAda.append(para)
                rel_name_noada.append(name)
        ...
        self.optimizer = optim.AdamW([
            ...
            {'params': rel_encoder_noAda, 'lr': float(config.LR), 'weight_decay': self.config.W_DECAY,
             'amsgrad': self.config.AMSGRAD},
            {'params': rel_encoder_ada, 'lr': float(config.LR)*10, 'weight_decay': self.config.W_DECAY,
             'amsgrad': self.config.AMSGRAD},
        ...
        self.lr_scheduler = CosineAnnealingLR(self.optimizer, T_max=self.config.max_iteration, last_epoch=-1) #2.5580e-04
    ...
    def forward(self, obj_points, edge_indices, descriptor=None, batch_ids=None, istrain=False):
        ...
        # obj_feature = self.obj_encoder(obj_points.permute(0,2,1).contiguous())
    ...
    def forward1(self, obj_points, edge_indices, descriptor=None, batch_ids=None, istrain=False):
        ...
        # edge_low = self.rel_predictor_3d.forward1(gcn_edge_feature_3d)
    ...
    def process_train(self, obj_points, gt_cls, descriptor, gt_rel_cls, edge_indices, batch_ids=None,
                      with_log=False, ignore_none_rel=False, weights_obj=None, weights_rel=None):
        self.iteration += 1

        obj_logits_3d, rel_cls_3d, obj_logit_scale = self(
            obj_points, edge_indices.t().contiguous(), descriptor, batch_ids, istrain=True)
