...
from src.utils.norm_line import compute_single_pca_normal
...
def gen_descriptor(pts:torch.tensor):
    ...
    # centroid [n, 3]
    # normline = compute_single_pca_normal(pts)
...
class Gen_edge_descriptor(MessagePassing):#TODO: move to model
    ...
    def message(self, x_i, x_j):
        ...
        # epsilon = 1e-6
        # edge_feature[:, 11:14] = x_i[:, 11:14] - x_j[:, 11:14]+epsilon  # normline offset
        
        # edge_feature, *_ = self.ef(edge_feature.unsqueeze(-1))
        return edge_feature.unsqueeze(-1)

    def message1(self, x_i, x_j):
        # 1. 计算旋转不变的质心关系 (2维)
        centroid_offset = x_i[:, 0:3] - x_j[:, 0:3]  # [num_edges, 3]
        # 计算XY平面上的欧几里得距离
        dist_xy = torch.norm(centroid_offset[:, 0:2], p=2, dim=-1)
        # Z轴上的偏移保持不变
        dist_z = centroid_offset[:, 2]
        
        # 将两个标量组合成质心关系特征
        centroid_feature = torch.stack([dist_xy, dist_z], dim=-1) # shape: [num_edges, 2]

        # 2. 计算旋转不变的STD关系 (2维)
        std_offset = x_i[:, 3:6] - x_j[:, 3:6]  # [num_edges, 3]
        # 计算STD在XY平面差值的模
        std_dist_xy = torch.norm(std_offset[:, 0:2], p=2, dim=-1)
        # STD在Z轴上的差值
        std_dist_z = std_offset[:, 2]
        
        std_feature = torch.stack([std_dist_xy, std_dist_z], dim=-1) # shape: [num_edges, 2]

        # 3. 计算其他不变特征 (5维)
        # 为防止除零或log(0)错误，加入一个极小值epsilon
        epsilon = 1e-6
        
        # 尺寸对数比
        dims_ratio = torch.log(x_i[:, 6:9] / (x_j[:, 6:9] + epsilon))
        
        # 体积对数比
        volume_ratio = torch.log(x_i[:, 9] / (x_j[:, 9] + epsilon)).unsqueeze(-1)
        
        # 长度对数比
        length_ratio = torch.log(x_i[:, 10] / (x_j[:, 10] + epsilon)).unsqueeze(-1)
        
        # 4. 拼接所有旋转不变特征
        new_edge_feature = torch.cat([
            centroid_offset,  # [num_edges, 3] 添加原始centroid offset
            std_offset,      # [num_edges, 3] 添加原始std offset
            centroid_feature, 
            std_feature,
            dims_ratio,
            volume_ratio,
            length_ratio
        ], dim=-1) # 总维度: 3 + 3 + 2 + 2 + 3 + 1 + 1 = 15

        return new_edge_feature.unsqueeze(-1)  # [num_edges, 15, 1]
