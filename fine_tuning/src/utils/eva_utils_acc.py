...
def evaluate_topk_predicate(rels_preds, gt_edges, multi_rel_outputs, topk, confidence_threshold=0.5, epsilon=0.02):
    ...
    rel_names = [i for i in range(26)]
    scene_accuracy = {rel: [] for rel in rel_names}
    ...
    for rel in range(len(rels_preds)):
        ...
        temp_topk = sorted(temp_topk) # the len of temp_topk is equal to rels_target
        counter = 0

        for k, tmp in enumerate(temp_topk):
            res.append(tmp - counter)

            if len(rels_target)>0:
                scene_accuracy[rels_target[k]].append(tmp - counter)

            counter += 1
        #res += temp_topk
    return np.asarray(res), scene_accuracy
...
def read_json(split):
    ...
    selected_scans = set()
    if split == 'train' :
        selected_scans = selected_scans.union(read_txt_to_list('/home/honsen/honsen/SceneGraph/baseline/data/3DSSG_subset/train_scans.txt'))
        with open("/home/honsen/honsen/SceneGraph/baseline/data/3DSSG_subset/relationships_train.json", "r") as read_file:
            data = json.load(read_file)
    elif split == 'val':
        selected_scans = selected_scans.union(read_txt_to_list('/home/honsen/honsen/SceneGraph/baseline/data/3DSSG_subset/validation_scans.txt'))
        with open("/home/honsen/honsen/SceneGraph/baseline/data/3DSSG_subset/relationships_validation.json", "r") as read_file:
            data = json.load(read_file)
    else:
