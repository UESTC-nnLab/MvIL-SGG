...
def self_supervise_aug(scene_points, instances):
    ...
    rotated_points = rotate_point_cloud_z(scene_points, 90)
...
def dataset_loading_3RScan1(root:str, pth_selection:str,split:str,class_choice:list=None):
    # read object class
    pth_catfile = os.path.join(pth_selection, 'classes.txt')
    classNames = util.read_txt_to_list(pth_catfile)
    # read relationship class
    ...
    selected_scans_valid = set()
    data = {}
    if split == 'train_scans' :
        selected_scans_train = selected_scans_train.union(util.read_txt_to_list(os.path.join(pth_selection,'train_scans.txt')))
        with open(os.path.join(root, 'relationships_train.json'), "r") as read_file:
            data1 = json.load(read_file)

        # selected_scans_valid = selected_scans_valid.union(util.read_txt_to_list(os.path.join(pth_selection,'validation_scans.txt')))
        with open(os.path.join(root, 'relationships_validation.json'), "r") as read_file:
            data2 = json.load(read_file)

        selected_list = set(["75c25989-9ca2-2844-97b4-31b81f7554b8","c7895f2b-339c-2d13-8248-b0507e050314",
                             "c92fb5b5-f771-2064-8570-dbe16cb33764", "f2c76fed-2239-29d0-8598-9ed42cec9dc5",
                             "6bde607d-9162-246f-8f84-98cf7ac2374c", "10b17940-3938-2467-8a7a-958300ba83d3"])

        data2 = select_scans(data2)
        selected_scans = selected_scans_train.union(selected_list)
        data['scans'] = data1['scans']+data2['scans']

    elif split == 'validation_scans':
        selected_scans = selected_scans_valid.union(util.read_txt_to_list(os.path.join(pth_selection,'validation_scans.txt')))
        with open(os.path.join(root, 'relationships_validation.json'), "r") as read_file:
            data = json.load(read_file)
...
#75c25989-9ca2-2844-97b4-31b81f7554b8  c7895f2b-339c-2d13-8248-b0507e050314  c92fb5b5-f771-2064-8570-dbe16cb33764  f2c76fed-2239-29d0-8598-9ed42cec9dc5 len 10



def select_scans(data):
    scans = data['scans']
    idx = []
    for i in range(len(scans)):
        scan = scans[i]
        scan_name = scan['scan']
        if scan_name!="75c25989-9ca2-2844-97b4-31b81f7554b8" and scan_name!="c7895f2b-339c-2d13-8248-b0507e050314" and \
                scan_name!="c92fb5b5-f771-2064-8570-dbe16cb33764" and scan_name!="f2c76fed-2239-29d0-8598-9ed42cec9dc5" and\
                scan_name!="6bde607d-9162-246f-8f84-98cf7ac2374c" and scan_name!="10b17940-3938-2467-8a7a-958300ba83d3":
            idx.append(i)
...
class SSGDatasetGraph(data.Dataset):
    def __init__(self,
        ...
        freq = o_rel_cls/o_rel_cls.sum()
        #
        # np.save("/home/honsen/honsen/SceneGraph/baseline/statictis_Triplet/train_o_rel_cls.npy",o_rel_cls)
    ...
    def __getitem__(self, index):
        ...
        map_instance2labelName = self.objs_json[scan_id] # object list in current scene split : {object_id:"object name"}
        ...
        # points,instances = self_supervise_aug(points,instances)

        obj_points, rel_points, gt_rels, gt_class, edge_indices, descriptor = \
            self.data_preparation(points, instances, 128, self.mconfig.num_points_union,
        ...
        # v1 = obj_points.view(-1, 3).detach().cpu().numpy()
        #
        # visualPoints_plt(v1)
        # indices = np.where(
        #     (instances == 1) | (instances == 42) | (instances == 4) |
        #     (instances == 25) | (instances == 26) | (instances == 44) |
        #     (instances == 13) | (instances == 14) | (instances == 15)
        # )[0]
        #
        # visualPoints_plt(points[indices])
        # print()
        while(len(rel_points) == 0 or gt_rels.sum()==0) and self.for_train:
            ...
            if self.use_descriptor:
                if self.use_2d_feats:
                    ...
                    obj_points, rel_points, gt_class, gt_rels, edge_indices, descriptor, scan_id = self.__getitem__(index)
            else:
    ...
    def data_preparation(self, points, instances, num_points, num_points_union, scene_id="",
                     ...
                     padding=0.2, num_max_rel=-1, shuffle_objs=True, all_edge=True, use_2d_feats=False, multi_view_root=None): #points: all point coord in the scene, instances: the label of each point
        #all_edge = for_train
        # get instance list
        ...
        for i, instance_id in enumerate(nodes):
            ...
            label_node.append(classNames.index(instance_name)) # refer to classes.txt, all object was arranged in this file
            # get node point
        ...
        return obj_points, rel_points, gt_rels, label_node, edge_indices, descriptor
