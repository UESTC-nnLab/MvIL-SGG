...
#obj_points, rel_points, gt_class, gt_rels, edge_indices, descriptor
def collate_fn_mmg(batch):
    ...
    return torch.cat(obj_point_list, dim=0), torch.cat(rel_point_list, dim=0), torch.cat(obj_label_list, dim=0), \
         torch.cat(rel_label_list, dim=0), torch.cat(edge_indices, dim=0), torch.cat(descriptor, dim=0), torch.cat(batch_ids, dim=0),scan_ids
