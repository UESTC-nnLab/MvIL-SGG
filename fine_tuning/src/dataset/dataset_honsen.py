...
class HonsenDatasetGraph(data.Dataset):
    def __init__(self,
        ...
        self.sample_list = constructSamplelist(self.root_ScanNet, self.scene_lists)
    ...
    def __getitem__(self, index):
        ...
        rotated_points = rotate_point_cloud_z(points, random_angle)
        ...
        obj_points, rel_points, edge_indices, descriptor = \
            self.data_preparation(points, instances, self.num_points, self.num_points_union, selected_rels,
                 padding=0.2)

        rotated_obj_points, rotated_rel_points, _, rotated_descriptor = \
            self.data_preparation(rotated_points, instances, self.num_points, self.num_points_union, selected_rels,
                                  padding=0.2)

        while (len(rel_points) == 0 or filtered_pesduo_labels == []) and self.for_train:
            index = np.random.randint(self.__len__())
            if self.use_descriptor:
                obj_points, rel_points, gt_rels, edge_indices, descriptor = self.__getitem__(index)
            else:
    ...
    def clip_label(self,selected_rels, filtered_pesduo_labels):
        ...
        for rel in selected_rels:
            ...
            # mean_embedding = torch.mean(text_features,dim=0)
